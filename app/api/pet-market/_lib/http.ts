...
export function mapPetMarketErrorToResponse(error: unknown) {
  ...
  const fallbackStatus = message.includes("must be") || message.includes("patch") ? 400 : 500;
  return jsonError(message, fallbackStatus);
}

export async function handlePetMarketRoute<T>(handler: () => Promise<T>) {
...
export async function handlePetMarketMutationRoute<T>(handler: () => Promise<T>) {
  const configError = ensureMongoConfigured();
  if (configError) return configError;
