...
import { mapPetMarketProductsToCpProducts } from "./mapPetMarketToCpProduct";
import type { Product } from "./companionPetData";

export function useCpPetMarketProducts(query: PetMarketProductSearchQuery = {}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (cancelled) return;
        setResult(data);
        setProducts(mapPetMarketProductsToCpProducts(data.items));
      } catch (err) {
        if (cancelled) return;
        const message = consumePetMarketApiError(
          err,
          "Failed to load products",
          CP_PATHS.databaseError,
        );
